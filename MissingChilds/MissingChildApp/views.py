...
def ParentLoginAction(request):
    if request.method == 'POST':
        ...
        with con:    
            cur = con.cursor()
            cur.execute("select username, password, email FROM parentsignup")
            rows = cur.fetchall()
            for row in rows:
      ...
      with con:    
          ...
          for row in rows:
              if row[0] == age and row[1] == colour:
                  index = 1
      if index == 1:
          imgs = ['child-1.PNG', 'child-2.PNG', 'child-3.PNG', 'child-4.PNG', 'child-5.PNG'] + [f'child-{i}.PNG' for i in range(6, 2169)]
          output = '<table border=1 align=center>'
...
def UploadAction(request):
    if request.method == 'POST' and request.FILES['t5']:
        ...
        # Define file path to save the uploaded image
        save_dir = 'C:/Python/MissingChilds/MissingChildApp/static/photo/'
        full_image_path = os.path.join(save_dir, f"{child_name}.jpg")

        # Save the image file
        fs = FileSystemStorage()
        fs.save(full_image_path, myfile)

        # Retry loading the image until it's ready
        max_attempts = 5
        attempt = 0
        frame = None
        ...
        # Still not loaded?
        if frame is None:
            context = {'data': 'Error loading image. Please try again after some time.'}
        ...
        db_connection = pymysql.connect(
            ...
            user='root',
            password='root',
            database='MissingChildDB',
            charset='utf8'
        )
        db_cursor = db_connection.cursor()
        insert_query = """
            INSERT INTO missing (person_name, child_name, contact_no, location, image, upload_date, status)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        db_cursor.execute(insert_query, (
            person_name, child_name, contact_no, location,
            os.path.basename(full_image_path), current_time, status
        ))
        db_connection.commit()
        print(db_cursor.rowcount, "Record Inserted")
